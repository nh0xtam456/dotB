...
<table id="tableorder" style="width:100%;background-color:#f6f6f6;">
   ...
   <div class="alert alert-info" role="alert">
      This Customer Also Buy...
   </div>
   <table id="chartCustomer" class="table">
   <thead>
     <tr>
   ...
   <tbody>
   {section name="item" loop=$order_cus_bought}
     <tr>
       <th><a href="http://localhost/SugarCE-Full-6.5.26/SugarCE-Full-6.5.26/index.php?module=A_Orders&action=DetailView&record={$order_cus_bought[item].id}">{$order_cus_bought[item].order_id}</th>
       <td>{$order_cus_bought[item].total_price}</td>
     </tr>
     {/section}
   </tbody>
   <tfoot>
     <tr>
         <th colspan="2" style="text-align:center;">Customer <em style="color:red;">{$bean->customer}</em> has spent {$total_price_spent} </th>
     </tr>
   </tfoot>
 </table>

<script src="custom/modules/A_Orders/js/detail.js"></script>