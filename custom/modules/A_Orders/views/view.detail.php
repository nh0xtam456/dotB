...
class A_OrdersViewDetail extends ViewDetail
{
 public function display()
 {
    $this->ss->assign("time_datecreate",date("d/m/Y"));     
    global $db;
    ...
    if(!empty($this->bean->order_id))
    {
      ...
      $sqlSelect1 = " SELECT *, p.price as unit_price, o.price as total_price, p.name as product_name
    ...
    $a=$this->bean->customer;
    $sqlSelect3 = "SELECT order_id, total_price,id FROM a_orders WHERE customer='$a' ";
    $result3 = $db->query($sqlSelect3);
    $total_price_spent='0';  
        //Duyệt qua từng dòng kết quả
        while($row3 = $db->fetchByAssoc($result3)){
            //Chuyển kết quả select sang dạng array
            $total_price_spent +=$row3['total_price'];
            $row3['total_price']=number_format($row3['total_price'],0,'',',').' VND';
            $arrayResult3[] = $row3;
        }
    $this->ss->assign("order_cus_bought",$arrayResult3);
    $this->ss->assign("total_price_spent",number_format($total_price_spent,0,'',',').' VND');
